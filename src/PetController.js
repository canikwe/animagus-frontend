class PetController {
  ...
  static manyPets() {
    Adapter.getAllPets().then((pets) => {
      pets.forEach((p) => {
        //instantiate each pet returned from the database
        const pet = new Pet(p);
        PetView.renderPetBtn(pet).addEventListener("click", () => {
  ...
  static gameEventListeners(pet) {
    ...
    pet.pet_characteristics.forEach((char) => {
      PetView.controlPanel(char, pet).addEventListener("click", (e) => {
  ...
  static handleCharacteristic(characteristic, pet) {
    ...
    const petCharData = {
      action_status: true,
      check_time: characteristic.check_time,
    };
    ...
    if (currentPet.active_status) {
      ...
      hapInterval = setInterval(() => {
        if (currentPet.happiness <= 0) {
          ...
          Adapter.updatePetDB(currentPet.id, {
            active_status: currentPet.active_status,
            happiness: 0,
          });
  ...
  static checkCharacteristic(pet) {
    pet.pet_characteristics.forEach((c) => {
      if (new Date() >= c.check_time) {
        ...
        const petData = {
          active_status: pet.active_status,
  ...
  static attachBackBtnListener(hapInterval) {
    PetView.renderBackBtn().addEventListener("click", () => {
      ...
      Pet.all.forEach((p) => {
        PetView.renderPetBtn(p).addEventListener("click", () => {
  ...
  static attachGalleryListeners() {
    ...
    for (i = 1; i <= 20; i++) {
      document
        .querySelector(`#pet-button-${i}`)
        .addEventListener("click", PetController.handleClickOfPetImage);
    }
  }
  ...
  static attachFormListeners() {
    document
      ...
      .addEventListener("submit", PetController.handleFormSubmit);
  }
  ...
  static handleClickOfPetImage(event) {
    ...
    document.querySelector("#image-input").value = document.querySelector(
      `#pet-image-${id}`
    ).src;
    document.querySelector("#main-pet").src = document.querySelector(
      `#pet-image-${id}`
    ).src;
  }
}
